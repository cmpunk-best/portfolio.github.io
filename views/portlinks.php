...
<hr style="display: block;
    ...
    border-width: 1px;"><br/>
     <?php if(count($linked)):?>
                    ...
                    <?php $images=CS50::query("SELECT * FROM userprof WHERE user_id=?",$linked[$i]["reqfor"]);
        $links=CS50::query("SELECT * FROM userinfolink WHERE user_id=?",$linked[$i]["reqfor"]);
        $name=CS50::query("SELECT * FROM userinfopersonal WHERE user_id=?",$linked[$i]["reqfor"]);?>
                        <ul class="list-group" id="contact-list">
